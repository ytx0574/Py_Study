...
class StaticFileRoute(object):
    ...
    def __call__(self, *args):
        fpath = os.path.join(ctx.applicaiton.document_root, args[0])
...
class MultipartFile(object):
    def __init__(self, stroage):
        self.filename = _to_unicode(stroage.filename)
        self.file = stroage.file
        ...
        if not hasattr(self, '_cookies'):
            ...
            if cookie_str:
                for c in cookie_str.split(';'):
                    pos = c.find('=')
                    if pos > 0:
                        cookies[c[:pos].strip()] = _to_unicode(c[pos+1:])
...
def _build_pattern_fn(pattern):
    ...
    raise ValueError('Invalid pattern definition in interceptro.')
...
class WSGIApplication(object):
    ...
    def add_module(self, module):
        self._check_not_running()
        m = module if type(module) == types.ModuleType else _load_module(module)
    ...
    def get_wsgi_application(self, debug=False):
        ...
        _applcation = Dict(document_root=self._document_root)
        ...
        fn_eexc = _build_interceptor_chain(fn_route, *self._interceptors)

        def wsgi(evn, start_response):
            ctx.applicaiton = _applcation
            ctx.request = Request(evn)
